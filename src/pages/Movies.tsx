...
export default function Movies() {
  ...
  async function loadResults() {
    try {
      let apiUrl = `${conf.TMDB_API}/discover/movie?api_key=${conf.TMDB_API_KEY}&sort_by=popularity.desc&include_adult=false&include_video=false&page=${page}`;

      if (selectedGenre) {
        // Fetch genre ID from TMDB using the genre name
        const genreReq = await fetch(
          `${conf.TMDB_API}/genre/movie/list?api_key=${conf.TMDB_API_KEY}`
        );
  ...
  return (
    <Fragment>
      ...
      {/* Genre Dropdown */}
      {/* <div className="genre-dropdown mx-4"></div> */}
            ...
            {results.map((v, i) => {
              return (
                <Link
                  ...
                  key={i}
                  title={v.title}
                  to={`/movie/${v.id}`} // Use a proper URL path based on your routing setup
                  style={{
